...
Changes over 2.7:
  (1) BACKOFF bumped to 10s base (was 2s). Retry sequence: 10s, 20s, 40s.
      Reduces false-error noise on slow reasoning models and Azure throttles.
  (2) kimi-azure substrate added: routes Kimi-K2.5 through Azure OpenAI
      endpoint (same AZURE_OPENAI_ENDPOINT / AZURE_OPENAI_KEY env vars).
      Existing kimi substrate retained for direct Moonshot API (KIMI_API_KEY).

Previous fixes retained from 2.4/2.5/2.6/2.7:
...
def run_battery(substrate: str, model_name: str, passes: int = 5,
    ...
    # kimi and kimi-azure are treated as different substrates for guard purposes.
    ...
    print(f"\n{'='*70}")
    print(f"BATTERY v2.8  |  {len(all_tests)} tests × {passes} passes  |  {model_name}")
...
def archive(results: Dict[str, Any], out_dir: str) -> str:
    ...
    path = os.path.join(out_dir, f"convergence_v28_{sub}_{ts}.json")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("usage: convergence_battery_v2_8.py <substrate> [model] [passes] "
