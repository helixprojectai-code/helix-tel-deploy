...
Changes over 2.8:
  (1) EMPTY RESPONSE GUARD: blank or whitespace-only responses from the model
      are now treated as None (excluded from γ) rather than being passed to
      verdict_rule or the judge. Blank responses were causing objective tests
      to return False (failed rule check on empty string) instead of None,
      inflating γ_objective with harness noise rather than model instability.
      Applies to both _objective_verdict and Judge.verdict.

Previous fixes retained from 2.4–2.8:
  ...
  - HTTPError retry on 401, 429, 500, 502, 503, 504.
  - Backoff 10s base (10s, 20s, 40s).
  - kimi-azure substrate for Kimi-K2.5 via Azure; kimi for direct Moonshot API.
"""
...
def _is_empty(text: str) -> bool:
    """True if response is blank/whitespace — treat as None, not False."""
...
class APIClient:
    ...
    def _post(self, url: str, headers: dict, payload: dict) -> str:
        ...
        Retries on: ConnectionError, Timeout, HTTPError 401/429/5xx.
        Fails fast on: 400, 403, 404, bad JSON.
        """
        last_err = None
...
class KimiDirectClient(APIClient):
    """Kimi via direct Moonshot API. Requires KIMI_API_KEY."""
...
class KimiAzureClient(AzureOpenAIClient):
    """Kimi-K2.5 routed through Azure OpenAI. Uses AZURE_OPENAI_ENDPOINT / AZURE_OPENAI_KEY."""
...
def make_client(substrate: str, model: Optional[str] = None) -> APIClient:
    ...
    if substrate == "kimi":
        return KimiDirectClient(model=model or "moonshot-v1-32k")
...
class Judge:
    ...
    Returns one of: True (pass), False (fail), or None (unparseable/error/blank).
    ...
    def verdict(self, response: str, rubric: str) -> Tuple[Optional[bool], str]:
        ...
        # v2.9: blank response → None, not a judge call
        if _is_empty(response):
            return None, "skipped: model response was empty"
...
def _objective_verdict(test: Dict, response: str) -> Tuple[Optional[bool], str]:
    ...
    # v2.9: blank response → None, not False
    if _is_empty(response):
...
def run_battery(substrate: str, model_name: str, passes: int = 5,
    ...
    print(f"\n{'='*70}")
    print(f"BATTERY v2.9  |  {len(all_tests)} tests × {passes} passes  |  {model_name}")
...
def archive(results: Dict[str, Any], out_dir: str) -> str:
    ...
    path = os.path.join(out_dir, f"convergence_v29_{sub}_{ts}.json")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("usage: convergence_battery_v2_9.py <substrate> [model] [passes] "
              "[judge_substrate] [judge_model]")
        print("  substrate / judge_substrate: local | azure | deepseek | kimi | kimi-azure")
        sys.exit(1)
